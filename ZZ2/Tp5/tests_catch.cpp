...
TEST_CASE("Constructeur par defaut") {
   Chaine c;
   CHECK( 0 == c.getCapacite()); // vous pouvez mettre 0 si vous preferez
...
TEST_CASE("Surcharge []") {
    ...
    s[0] = 'r';
    CHECK (s[0] == 'r');
    CHECK(strcmp(s.c_str(),"rne nouvelle surcharge") == 0);
}

TEST_CASE("Surcharge +") {
    const Chaine s("Hello les ");
    const Chaine s1("ZZ2");
    const Chaine s2 = s + s1;

    CHECK( strcmp(s2.c_str(),"Hello les ZZ2") == 0);
}
