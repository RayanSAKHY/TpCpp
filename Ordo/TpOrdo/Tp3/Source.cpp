		...
		for (int i = 0; i < R1.taille; ++i) {
			...
			for (int r2 = 0; r2 < sol.nbTournee; ++r2) {
				...
				for (int j = 0; j <= R2.taille; ++j) {
					...
					const int z = (jj == R2.taille || r1 == r2) ? ((jj == R2.taille) ? 0 : R2.liste[jj]) : R2.liste[j];
					...
					if (total_delta < -EPS) {
						...
						if (R1.taille == 0) {
							for (int k = r1; k < sol.nbTournee - 1; ++k)
								sol.listeTournee[k] = sol.listeTournee[k + 1];
							sol.nbTournee--;
						}

						calculeCoutSolution(prob, sol);

						nbMoves++;

						recomputeRoute(prob, R1);
						recomputeRoute(prob, R2);
